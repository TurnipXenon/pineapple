...
<style lang="scss">
	...
	.pinya-toaster-root {
		.toast-instance {
			...
			transition:
				bottom 0.25s linear 0.2s,
				transform 0.25s linear 0.2s;

			&:last-child {
				// the entering new child will bounce with y2=1.2
				transition:
					bottom 0.25s cubic-bezier(0, 0, 0.5, 1.2),
			...
			left: 1rem;
			box-shadow: var(--toast-shadow-px) var(--toast-shadow-px) var(--toast-shadow-px)
				var(--shadow-color);
