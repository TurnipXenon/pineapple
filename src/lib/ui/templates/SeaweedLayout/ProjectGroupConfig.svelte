...
>
	<div class="m-4 flex flex-row gap-2">
		<button title={`Remove ${group.title}`} onclick={removeGroup(group)}>
			<img class="img-icon" src={CloseIcon} alt={`Remove ${group.title}`} />
		</button>

		<button
			title={`Move group ${group.title} upwards`}
		...
		<button
			title={`Move group ${group.title} downwards`}
	...
	<table class="table">
		...
		<tbody>
			{#each group.entryList as entry, entryIndex (entry)}
				<tr class="entry-row">
					<td class="entry-data">
						<button title={`Remove ${entry.key}`} onclick={removeEntry(entry, group)}>
							<img class="img-icon" src={CloseIcon} alt={`Remove ${entry.key}`} />
						</button>

						<button
							title={`Move ${entry.key} upwards`}
						...
						<button
							title={`Move ${entry.key} downwards`}
		...
		<tfoot>
			<tr>
				<td class="combobox-container">
					<PinyaButton onclick={addEntry}>Add Entry</PinyaButton>
					<PinyaCombobox
