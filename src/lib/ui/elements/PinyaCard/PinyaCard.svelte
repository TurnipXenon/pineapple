...
{#if _props.includeDataNoSnippet}
	<div id={_props.id} data-nosnippet class={`card text-start rounded-xl pinya-card ${cardClass}`}>
		{@render _props?.children?.()}
	</div>
{:else}
	<div id={_props.id} class={`card text-start rounded-xl pinya-card ${cardClass}`}>
		{@render _props?.children?.()}
	</div>
{/if}
