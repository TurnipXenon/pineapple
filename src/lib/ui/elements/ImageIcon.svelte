...
<script lang="ts">
	...
	const { colorScheme = ColorScheme.Primary, ...props }: PinyaButtonProps = $props();
...
<img {...props} class={`image-icon ${tailwindClass}`} />
...
<style>
	...
	:global(.icon-filter-on-primary-button) {
		...
		filter: brightness(0) saturate(100%) invert(18%) sepia(10%) saturate(4620%) hue-rotate(331deg)
	...
	@media (prefers-color-scheme: dark) {
		:global(.icon-filter-on-primary-button) {
			/* based on surface-0 */
			filter: brightness(0) saturate(100%) invert(85%) sepia(3%) saturate(993%) hue-rotate(326deg)
