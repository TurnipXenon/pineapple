 ...
 * Unvisits a node
 */
export class ForceVisitCommand implements LineBehaviorNode {
	process(nodeArgs: LineNodeArguments): LineBehaviorResult {
		if (!nodeArgs.line.startsWith("<<forceVisit") || (!browser && !import.meta.env.VITEST)) {
			return {
				renderedLine: "",
		...
		const nodeName = nodeArgs.line
			.slice("<<forceVisit ".length, nodeArgs.line.length - ">>".length)
		...
		const key = `+${nodeName}`;
		if (!nodeArgs.dialogVariableStore.getItem(key)) {
			nodeArgs.dialogVariableStore.setItem(key, "1");
		}
