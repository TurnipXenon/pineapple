...
class ExpressionEvaluator {
	...
	evaluateFreeform = (line: string, prefix: string): string[] => {
		...
		for (const character of expression) {
			if (escapeToggle) {
				...
				if (currentToken !== "") {
					...
					if (
						currentToken[0] !== "$" && // should not be a variable stand-in
		...
		infixTokens.forEach((token) => {
			if (token.startsWith("#")) {
				...
				while (btreeUtils.peek(stack) !== "(") {
					...
					if (stack.length === 0) {
						console.error(
							"Assumption was made in postfix conversion that stack will not be empty! It was wrong!"
						);
						throw new Error("Wrong postfix conversion!");
					}

					postfixTokens.push(<string>stack.pop());
				}
		...
		// evaluate
		let currentState: ExpressionState = {
			operandStack: []
		};
